-- VNX Migration 0023 — tracks tenant-scoping (ADR-007 compliance)
--
...
-- Post-migration state (v23): composite PKs + FKs over (track_id, project_id).
--
...
--
-- Idempotency: apply_script_if_below skips entire script when user_version >= 23.
...
-- v23 tracks has: PRIMARY KEY (track_id, project_id) — ADR-007 composite
--
...
ALTER TABLE tracks RENAME TO tracks_pre_v23;
...
-- v23: adds project_id, composite FK (track_id, project_id) → tracks
--
-- project_id column not present in v22; stamp all existing rows as 'vnx-dev'.
-- sqlite_sequence preservation required (kimi peer-review §1 — AUTOINCREMENT hazard).

ALTER TABLE track_phase_history RENAME TO track_phase_history_pre_v23;
...
SELECT
    id, track_id, 'vnx-dev',
    from_phase, to_phase, actor, reason, approval_id, occurred_at
FROM track_phase_history_pre_v23;
...
-- v23: (from_track_id, from_project_id, to_track_id, to_project_id) composite PK
--      Cross-project deps allowed (design §2.4 — JA toelaten)

ALTER TABLE track_dependencies RENAME TO track_dependencies_pre_v23;
...
SELECT
    from_track_id, 'vnx-dev', to_track_id, 'vnx-dev',
    kind, derivation_source, confidence,
    COALESCE(evidence_json, '{}'), derived_at
FROM track_dependencies_pre_v23;
...
-- v23: (track_id, project_id, oi_id, link_type) composite PK

ALTER TABLE track_open_items RENAME TO track_open_items_pre_v23;
...
SELECT
    track_id, 'vnx-dev', oi_id, link_type, link_source, linked_at
FROM track_open_items_pre_v23;

-- ============================================================================
-- STEP 5: DROP renamed pre-v23 tables (clean up)
-- ============================================================================

DROP TABLE track_open_items_pre_v23;
DROP TABLE track_dependencies_pre_v23;
DROP TABLE track_phase_history_pre_v23;
DROP TABLE tracks_pre_v23;
