...
**Success criteria**:
- [ ] 3 agent directories exist under `agents/`
- [ ] Each has a CLAUDE.md with clear role definition and constraints
- [ ] Each has a config.yaml pointing to `light` governance profile
- [ ] Subprocess adapter resolves agent dirs correctly (agents/{role}/ cwd)
...
**Success criteria**:
- [ ] `vnx dispatch --agent blog-writer` works end-to-end
- [ ] Agent runs with light governance profile (exception-only review)
- [ ] Agent output lands in correct unified_reports/ location
- [ ] Folder scope isolation prevents cross-scope access
...
**Success criteria**:
- [ ] blog-writer produces a real blog post
- [ ] linkedin-writer produces a real LinkedIn post  
- [ ] research-analyst produces a real research summary
- [ ] Agent creation guide exists
- [ ] All governance records complete (receipts, reports, gate records)
