...
| Same work, but a human needs a live pane to watch or intervene | `tmux_interactive_dispatch.py` (`claude_tmux_subscription`) — explicit opt-out via `--force-tmux` + reason (mirrors `--allow-headless`), leaseless, isolated worktree, subscription-safe, fresh checkout/dispatch |
