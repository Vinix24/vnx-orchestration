...
PR/gate work is routed through `vnx dispatch`, which selects the lane; the single-entry door is the default single funnel for that selection (ADR-024, see header). `tmux_interactive_dispatch.py` is the claude lane; T1/T2/T3 build workers default to the `kimi` provider lane instead (`workers-kimi-pinned` — see §8), so `tmux_interactive_dispatch.py --model sonnet` now only applies to an explicit claude override. Other defaults: `--isolated-worktree` on, `--base-ref origin/main`, `deadline_seconds` 3600 (receipt-wait ceiling, `stage_spec_bundle`); staging gate via `--from-staging-id` (ADR-006). Required: `--dispatch-id`, `--instruction`.
