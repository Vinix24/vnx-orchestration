...
- **Subscription reviewers first (operator decision 2026-09-26):** the default review stack is `codex_gate,kimi_gate` (`VNX_DEFAULT_REVIEW_STACK`), both on a subscription (codex CLI, kimi CLI OAuth). `glm_gate` (OpenRouter credit) and `deepseek_gate` (API credit) are not standing seats: they read a PR only when both subscription seats are unavailable, through the takeover chain `codex_gate,kimi_gate,glm_gate,deepseek_gate` (`VNX_REVIEW_GATE_TAKEOVER_CHAIN`). `gate_recorder.GATE_BILLING` classifies every gate, and `tests/test_review_stack_subscription_first.py` fails when the default stack or the chain puts an API-credit gate before a subscription gate. `vnx gate <pr> --only kimi` runs kimi through the same governed lane as any other gate (`gate_runner`'s harness lane, since v1.6.2), so it no longer books the `not_executable` / `gate_not_subprocess_routable` record of a script-runner registration. When codex is at its limit the codex seat goes to kimi and kimi is requested once, not once per seat.
- **A truncated diff is not a pass (OI-1851):** glm_gate/kimi_gate (standalone and via `gate_runner`'s harness lane) cut the diff at `gate_lane_contract.MAX_DIFF_CHARS` (50.000). Every record carries the real size in `execution_depth` (`diff_chars`, `diff_truncated`, `diff_limit`, `truncated_files`). A clean verdict on a cut diff is `partial_review`, unless an agentic run with a parsed tool stream read at least one file per cut file; a fail stays a fail. The merge door gives NO-GO on it, and on any record showing an unread truncation, naming the size. Way out: a peer's full pass on the same head (e.g. `codex_gate`, uncapped), or split the PR. Under the cap nothing changes.
- **CI workflow conclusion (mandatory):** `gh run list --branch <head> --workflow "VNX CI" --limit 1 --json conclusion --jq '.[0].conclusion'` must equal `success`. `gh pr checks` listing green names is NOT sufficient — a multi-step job can still produce a `failure` conclusion. `VNX CI` is the fabric's own workflow; a project names its own with `ci_workflow` in its `branch_protection.yaml` on main (else `VNX_CI_WORKFLOW_NAME`, else `VNX CI`). No `enforcement` setting softens this gate, and a PR that changes, adds or removes `ci_workflow` needs `--allow-weaken "<reason>"`, because it would choose the workflow every later PR is judged on.
...
- **Branch-protection preflight (golf B, B1):** `pr_merge.py` refuses a merge when live branch protection on `main` drifts from the project's `branch_protection.yaml`, or when the PR's own copy of that YAML weakens it (`--allow-weaken "<reason>"` is the only override). "The project" is the repo the merge goes into (`VNX_PROJECT_ROOT`, else the git toplevel of the cwd; the first output line is `doelrepo: owner/name`), and how strictly it is held is the project's call (`enforcement: enforce | warn | off`, OI-1849). Operator setup (GitHub App `vnx-gate`, keychain, first apply, migration, rollback) and how a consumer project sets this up: `docs/operations/FORGE_GATE.md`.
- **De samenvattende check `vnx-gate/review` (golf B, B3):** naast één check-run per poort (`vnx-gate/glm_gate`, ...) publiceert `scripts/lib/forge_gate_publisher.py review --pr <n>` één check-run per head die de enige vraag beantwoordt die branch protection nodig heeft: ligt er een geldig review-akkoord op déze commit. De regel is niet opnieuw opgeschreven — hij is `closure_verifier._REVIEW_PEER_GATES` + `check_review_gate_for_merge`, dezelfde waarheid als de merge-deur, inclusief de overnameketen (OI-1576) en de uitsluiting van `ci_gate` als ondertekenaar (OI-1645). `success` alleen bij een bewezen pass op deze kop, `failure` als er wél iets beslissends is gezegd maar geen akkoord (een kale `ci_gate`-pass valt hieronder), `action_required` bij afwezigheid, uitval of een oordeel over een andere commit. Nooit `neutral` of `skipped`: GitHub leest die op een verplichte check als VOLDAAN. De check staat in `pending_checks` en is dus nog niet verplicht — `pending-preview` toont het exacte PUT-object dat de promotie oplevert.
...
- **Measurement (#1249):** `check_pr_size` (`scripts/pre_merge_gate.py`) measures `merge_base(base, head)..head`, and fails loud when the base cannot be resolved. It previously measured `git diff HEAD~1 HEAD` — the dirty working copy relative to one commit back — which over/under-counted whenever local HEAD carried commits the PR itself doesn't (258 reported lines for a PR that actually had 110, including 50 deletions not in the PR).
...
| Task | Lane |
|---|---|
| Parallel / independent claude feature work (default since A2, 2026-08-26) | `dispatch_envelope.run_envelope_headless_plan` (`claude_headless`) — `claude -p` via envelope, isolated worktree, subscription-safe |
| Same work, but a human needs a live pane to watch or intervene | Not offered. The tmux-spawn lane was removed on 2026-09-18 (`docs/operations/TMUX_SPAWN_LANE.md`) |
| Terminal-pinned single-worker PR (Wave 5 smart-context, lease, triple-gate) | `subprocess_dispatch.py` |
| Work expected to run >30 min | `subprocess_dispatch.py` |
| PR review gate | `review_gate_manager.py` / `t0_gate_enforcement.sh` |
| Pure utility (no PR, no gate) | direct Bash |

PR/gate work is routed through `vnx dispatch`, which selects the lane; the single-entry door is the default single funnel for that selection (ADR-024, see header). `dispatch_plan.resolve_claude_lane()` is the single source of truth for the claude lane: a `provider=claude` spec resolves to `claude_headless`, with or without an explicit `allow_headless` (which needs a `headless_reason` and adds a "HEADLESS lane opted-in" plan warning). The tmux-interactive lane, its `force_tmux` opt-out and its `VNX_ALLOW_TMUX_LANE` brake were removed on 2026-09-18. A staged spec on disk that still carries `force_tmux: true` is ignored with a loud stderr warning and runs headless (`dispatch_cli._note_removed_lane_keys`); a spec that carries `force_tmux: false` is ignored with a log line. T1/T2/T3 build workers default to the `kimi` provider lane instead of the claude lane (`workers-kimi-pinned` — see §8), so the headless lane only applies to an explicit claude override. Door defaults: `base_ref` `origin/main`, `deadline_seconds` 3600 (receipt-wait ceiling, `stage_spec_bundle`); the staging gate is the staged bundle itself (ADR-006).

**History: why the default moved (measured, not a preference):** the envelope lanes behind `claude_headless` wired the OI-1115 duplicate-PR guard (`skip_pr`); the tmux lane never did until A3. Field consequence 25/26-08-2026: 3 of 3 dispatches built on an existing dispatch branch opened a second PR on the same sha (#1685, #1686, #1687). A3 (#1689, main `50255a3c`) closed that gap on the tmux side, and A2 (#1693, main `282de69b`) separately moved the default to headless. Headless also closes two governance gaps (isolation, report-before-receipt — see §8). Since 2026-09-18 headless is the only claude lane.

**Deadline override (20260716-deadline-passthrough):** the consumer-door `vnx dispatch-agent --deadline-seconds N` (300-14400, out-of-range hard-errors) overrides the 3600s default end-to-end via `dispatch_bridge.deliver_via_door(deadline_seconds=...)` -> `stage_spec_bundle(deadline_seconds=...)`. `bin/vnx dispatch` needs no separate flag — on the staged path it only ever consumes an already-staged `dispatch-spec.json`, so a bundle staged with a custom `deadline_seconds` (via `stage_spec_bundle` directly, per §12) already carries it through. The exception is the deprecated raw `.md` legacy lane (`scripts/commands/dispatch.sh` `cmd_dispatch`, `:288`/`:315`): a bare `vnx dispatch <file.md>` falls through to the legacy lane (ADR-025) and never reads a `dispatch-spec.json`, so it has no deadline override to carry.

**Worker permissions default (#1016, revised 14-08):** detached subprocess workers launch **scoped** by default — `--permission-mode acceptEdits` + empty ambient MCP + role allow-list from `.vnx/worker_permissions.yaml`. The old blanket `--dangerously-skip-permissions` default rested on the worktree argument, which weighed one axis and missed the MCP axis: worktree isolation bounds the filesystem, not the network, and an MCP server talks to a service outside the checkout (measured 14-08: six ambient MCP servers still connected from a dispatch worktree without opt-in). Blanket skip is now the explicit opt-out — `VNX_WORKER_BLANKET_SKIP=1`, or the legacy `VNX_WORKER_SCOPED=0`/`false`/`no`/`off`. A `working_tree_only` dispatch (plan-review/plan-write, no commit/push allowed) fail-closes: it refuses the opt-out paths where the commit/push deny would not bind. Full detail: `docs/operations/WORKER_PERMISSIONS.md`.
...
`claude-tmux` is the historical name of the lock class every claude dispatch holds (the tmux lane it was named after was removed on 2026-09-18). Claude runs on **subscription** sessions, which have a concurrent-session cap **shared with every other Claude agent on the account** (production agents, other terminals). Exceeding it = the dispatch immediate-exits in ~0.1s (`rc=1`), NOT a code error.

- **Default: serialize claude-tmux — N-slot, N=10 (operator directive 2026-08-21, dispatch-20260821-t0-tmux-concurrency-10; raised from N=5, operator directive 2026-08-11, dispatch-20260811c-b).** Provider lanes (codex/kimi/glm/deepseek) do NOT use the Claude subscription and stay parallel. The N=5 default itself was raised from the historical N=1 default once #1451 gave every tmux dispatch its own named paste buffer — concurrent dispatches used to cross each other's instructions through tmux's single shared paste buffer (measured 15/20 crossings at 4 simultaneous dispatches; 0/20 after #1451). Raising concurrency further is only sound on a fabric that carries that fix.
...
- **`VNX_TMUX_MAX_CONCURRENT` (N-slot semaphore, #1017; registry-backed since dispatch-20260821-t0-tmux-concurrency-10, `config_registry.py`)** — overrides the default. `serialize_lane` acquires the first free slot among `N` independent `flock`s on `<lock_dir>/claude-tmux-slot-{0..N-1}.lock` (`dispatch_serialization.py:69-81,188-256`). Missing, `0`, negative, or unparseable values clamp to `10` (the subscription-safe default) — only a valid positive integer overrides it with a different concurrency level. Precedence: the process env var wins first (an explicit per-session override); then the registry-backed config-store value (`project_config` via `config_runtime` — the same surface an operator's dashboard flips); then the `10` default. Raising it is an explicit, informed operator choice, not a default the code creeps towards: raising it further trades serialization safety for throughput against the account's real session cap. The lock dir is account-level (`$VNX_LOCK_DIR` or `~/.vnx-data/locks`), shared across every project on the account by design — setting `VNX_LOCK_DIR` per project to get 10 slots per project instead of 10 across the account multiplies past the real session cap.
...
- **Holder metadata is cleared on release (#1249).** `serialize_lane` writes holder metadata (pid/dispatch_id/timestamp) into the slot's lock file at acquire time; until #1249 it never cleared that metadata on release, so a freed slot's lock file still showed the prior holder and both `force_release` and any diagnostic reader saw it as OCCUPIED. It is now truncated on every release path, including the exception path, so a genuinely free slot reads as free.
- **Diagnostic:** a Claude cell that DNFs at ~0.1s with `rc=1` = session-cap/rate-limit (capacity), not a bug. A Claude cell with `cost=$0.0000` confirms the subscription lane (headless bills the subscription — see the headless note in §7/§8). Remedy: wait for the usage-window reset, free a session, or (deliberately) raise `VNX_TMUX_MAX_CONCURRENT`; re-run with `--retry-from`.
...
|---|---|---|
| Parallel claude: ~0.1s exits | subscription session-cap under concurrent claude load | Serialize claude-tmux (§6). |
| The whole Bash dispatch command is blocked by the PreToolUse hook | the spawn-guard greps T0's own Bash tool-call text for a raw spawn attempt; an inline `--instruction`/heredoc containing literal `claude -p`/`--print`, `kimi --print`/`-p`, `codex exec`, or `--dangerously-skip-permissions` trips it | Write the instruction to a file and pass `--instruction "$(cat /abs/path.md)"` — the hook sees the literal `$(cat …)` (no spawn token); the shell expands at run time. Keep the inline command free of those tokens. Note: the governed spawn injects its own permission posture (scoped `--mcp-config ... --strict-mcp-config` by default since 14-08; `--dangerously-skip-permissions` only when the operator opts back in via `VNX_WORKER_BLANKET_SKIP=1`) — that internal use is not what the hook is guarding against; it blocks the *literal token appearing in T0's own Bash command*, i.e. a raw/manual spawn attempt. |
...
| Claude silently bills API instead of subscription | a direct API key/base-url override bypassing the lane, or stale `HEADLESS_FORCED_MODELS` | The headless lane (opened 2026-08-11, `claude-headless` constraint) bills the subscription, not API credits — `cost=$0.0000` confirms it; nonzero means something bypassed it (e.g. `ANTHROPIC_API_KEY`/`ANTHROPIC_BASE_URL` set in the environment). |
...
| A fallback picks a seat or lane that cannot answer (quota spent, key refused) although its CLI or package is present | `is_available()` measured presence, not reachability (OI-1454) | Availability is presence AND not recorded unreachable. `scripts/lib/provider_reachability.py` keeps one record per provider in `<state_dir>/provider_reachability/<provider>.json`, written by real outcomes (gate results, provider-lane dispatches, codex/litellm adapter calls; classifier providers only read it) and expiring on its own (an unreachable record lives one quota cooldown, a reachable one 15 minutes). The review-gate takeover walk and `lane_available` skip a provider recorded unreachable and name the reason. `unmeasured` is never read as `reachable`. There is no probe: a probe run with the caller's environment does not carry the lane's credential (OI-1507). An operator who fixed the cause clears a record early by deleting its file. |
...
|---|---|---|
| sonnet / opus (claude) | `claude_headless` (the only claude lane, envelope `claude -p`) | Subscription. T0 is pinned to Opus (`t0-opus-only`). **T1/T2/T3 build workers default to sonnet** (`workers-kimi-pinned`, `pin_semantics: default`; operator decision 2026-09-23, confirmed 2026-09-26: sonnet is the standing default while there is usage). The id keeps its old name because `_load_model_pins_from_yaml()` (`scripts/lib/dispatch_cli.py`) keys on that exact string. `default` is advisory: a spec that names its own model or provider wins, so `kimi-k3` stays an explicit per-dispatch choice (see the kimi row below). A build-worker dispatch with `provider=claude` and no explicit model resolves through `_load_model_pins_from_yaml()` and the D4 model-tier step (`scripts/lib/dispatch_plan.py`) to `sonnet` and needs no override. `VNX_OVERRIDE_WORKERS_KIMI_PINNED=1` only downgrades the `workers-kimi-pinned` warning's log severity; it does not change routing. Headless `claude -p` has been the default claude lane since A2 (2026-08-26, #1693, main `282de69b`) and is the only one since 2026-09-18 — every `provider=claude` spec resolves to `claude_headless` via `dispatch_plan.resolve_claude_lane()`; the earlier 2026-08-11 operator directive only unblocked the lane (`claude-headless` constraint downgraded to warn, `lane_safety.headless_block.enabled: false`), A2 is what made it the default resolution. `VNX_OVERRIDE_CLAUDE_HEADLESS=1` re-lifts the headless block if it's ever re-enabled — a separate, older override, not the default toggle. The lane runs on the subscription, not API credits. Both governance gaps the old tmux default was hedging against are closed on headless: `isolation=worktree` is honored (`run_envelope_headless_plan()` creates the dispatch worktree via `create_dispatch_worktree()`, passes `cwd=wt_path` to the adapter, and hard-aborts rather than falling back to the main checkout on creation failure — PR #1416, main `9f32ae84`, 2026-08-08), the fail-closed report gate binds on envelope lanes before the receipt is emitted (PR #1420, main `c28b7ac4`, 2026-08-09), and the door stopped silently no-op'ing an isolation guarantee it couldn't verify (PR #1478, 2026-08-12). Measured on main 2026-08-21: `pytest -k HeadlessIsolationGuard` → 7 passed, `pytest tests/test_headless_worktree_isolation.py` → 6 passed. Serialize under load (§6, N-slot via `VNX_TMUX_MAX_CONCURRENT`, default 10 — the headless lane holds the `claude-tmux` serialization class, OI-1417). |
| codex (gpt-5.x) | `provider_dispatch.py --provider codex` | Has tools. Retry once on lane-launch DNF (`codex_retry_once`). |
| kimi (k2.x / k3) | `provider_dispatch.py --provider kimi` | **kimi CLI OAuth only** (`kimi-via-cli-only`). An explicit per-dispatch choice for T1/T2/T3 build work. It was the build-worker default from 2026-07-23 while the sonnet quota was spent; sonnet is the default again (sonnet row above). `routing_policy.yaml` has no fallback chain for the kimi lane, so a routing miss fails loud and never silently re-routes to claude/sonnet. The CLI OAuth serves `kimi-k3` (registry `default_model`, `cli_model_arg: kimi-code/k3`). NOT `litellm:moonshot` (bare API; violates the constraint; baseline-only). |
| GLM-5.2 / 5.3 / 5.3-flash | `provider_dispatch.py --provider litellm:zai` | Resolves to `openrouter/z-ai/<model>` + `OPENROUTER_API_KEY` → satisfies `zai-via-openrouter-only`. The default single-entry door normalizes this to the `glm-harness` lane (`glm-via-harness-only`). Allowlist lives in `provider_constraints.yaml` (`deprecated-glm-models`); GLM-5.2 is the default, GLM-4.5/4.6/5/5.1 rejected. |
...
|---|---|---|
| claude (subscription; measured over tmux + headless reports) | Yes | 262/262 |
...
2. **Stage a spec bundle into the CENTRAL pending dir** — either call `dispatch_bridge.stage_spec_bundle(instruction_text=..., dispatch_id=..., role=..., target_slot=..., provider=..., model=..., gate=..., dispatch_paths=..., tags=..., track_id=<track-id-or-None>, data_dir=~/.vnx-data/<project>)` from Python, or (OI-1639) use the stage-only CLI so an operator never has to drop into a Python shell to stage: `python3 scripts/lib/dispatch_bridge.py stage --instruction <instruction-file> --dispatch-id <id> --role <role> --slot T0|T1|T2|T3 [--provider claude] [--model sonnet] [--task-class implementation] [--deadline 7200] [--tag ...] [--track <track-id>] [--data-dir <path>]` (also wired as `vnx dispatch stage ...` in `scripts/commands/dispatch.sh`). It prints the resulting `dispatch-spec.json` path and **never fires** — unlike `dispatch_bridge.bridge_dispatch`/the legacy `dispatch_bridge.py` CLI (no `stage` first token), which stage AND immediately drive the bundle through `run_dispatch`. Either path resolves the data root via the same helper the door uses (`dispatch_cli._resolve_data_dir`) and writes `<data_dir>/dispatches/pending/<dispatch_id>/{instruction.md,dispatch-spec.json}` (`scripts/lib/dispatch_bridge.py:102-198`). Staging into a repo-local pending dir instead of the central one means `vnx dispatch <id>` won't find the bundle at bundle-existence check time (`_d_is_staged_form`, `scripts/commands/dispatch.sh:126-159`) and falls through to the deprecated legacy raw-file lane (ADR-025) — always stage central. **`gate` is validated (#1249)** against `REGISTERED_GATE_NAMES` (`scripts/lib/dispatch_spec.py`: the closed `Gate` enum plus `GATES_OUTSIDE_ENUM`, today `deepseek_gate`) — an unrecognized value fails loud instead of silently staging a dispatch that never runs a gate. That one set is what the staging bridge, the door's `validate()` Rule 16, the takeover-chain parser and the review-stack router (`smart_router._primary_review_gate`) all read; `tests/test_gate_name_registry.py` pins it against `gate_recorder.GATE_PROVIDERS`, the gates that can actually run. The FEATURE_PLAN.md phase labels `planning`/`implementation` (§1's example) are not gate names; they normalize to the same "no gate assigned" sentinel an absent `gate` already gets, they are not registered gates.
3. **Dry-run the door** — `bin/vnx dispatch <dispatch-id> --dry-run`. A bare dispatch-id with an existing bundle at `pending/<id>/dispatch-spec.json` routes to the door, not the legacy lane; `--dry-run` prints the compiled plan + permit fingerprint (lane/model/billing/route_reason) and spawns nothing. **`--track <track-id>` (OI-1639)** stamps `spec.track_id` at fire time — for a bundle staged without one, or to override one — before `validate()`/the TL-D1 check runs, so a nonexistent/done track still rejects loud exactly as it would if the staged spec itself had carried it; the dry-run print shows the resolved `track_id` line so a fire-time stamp is visible before it commits to anything. Any door flag the `dispatch.sh` wrapper does not itself enumerate (`--track`, `--refire`, `--reopen-lane`, `--reopen-reason`, `--override-stop-conditions`) is forwarded to `dispatch_cli.py` verbatim (OI-1639) — the wrapper is not a second flag list the door can drift out of sync with.
