...
|---|---|---|
| sonnet / opus (claude) | `tmux_interactive_dispatch.py` `--model sonnet`\|`opus` | Subscription. **T1/T2/T3 workers are pinned to `claude-sonnet-5`** (`workers-sonnet-pinned` constraint, bumped from 4.6 on 2026-07-05, #1013); T0 stays Opus. `_load_model_pins_from_yaml()` (`scripts/lib/dispatch_cli.py:190-213`) loads this unconditionally from `provider_constraints.yaml` — the constraint's `override_allowed: true` is declarative only; no `VNX_OVERRIDE_WORKERS_SONNET_PINNED` env var exists in the door code, so a plan-floor above the pin must escalate to the operator rather than assume an override will apply. Headless `claude -p` is opt-in and blocked by default (`claude-headless` constraint; `VNX_OVERRIDE_CLAUDE_HEADLESS=1` to open it, = API). Serialize under load (§6, N-slot via `VNX_TMUX_MAX_CONCURRENT`). |
