...
- **Default: serialize claude-tmux — N-slot, N=5 (operator directive 2026-08-11, dispatch-20260811c-b).** Provider lanes (codex/kimi/glm/deepseek) do NOT use the Claude subscription and stay parallel. Raised from the historical N=1 default now that #1451 gives every tmux dispatch its own named paste buffer — concurrent dispatches used to cross each other's instructions through tmux's single shared paste buffer (measured 15/20 crossings at 4 simultaneous dispatches; 0/20 after #1451). Raising concurrency is only sound on a fabric that carries that fix.
...
- **`VNX_TMUX_MAX_CONCURRENT` (N-slot semaphore, #1017)** — overrides the default. `serialize_lane` acquires the first free slot among `N` independent `flock`s on `<lock_dir>/claude-tmux-slot-{0..N-1}.lock` (`dispatch_serialization.py:69-81,188-256`). Missing, `0`, negative, or unparseable values clamp to `5` (the subscription-safe default) — only a valid positive integer overrides it with a different concurrency level. This is an explicit, informed operator choice, not a default the code creeps towards: raising it further trades serialization safety for throughput against the account's real session cap. The lock dir is account-level (`$VNX_LOCK_DIR` or `~/.vnx-data/locks`), shared across every project on the account by design — setting `VNX_LOCK_DIR` per project to get 5 slots per project instead of 5 across the account multiplies past the real session cap.
