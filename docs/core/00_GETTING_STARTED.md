...
- **Receipt Bulletproofing**: Full prevention and recovery for stuck receipts
...
- **Dual Format Support**: Both JSON and Markdown dispatches work without extra configuration
...
│                     T0 (Orchestrator)                           │
│  1. Creates dispatch                                            │
│  2. Writes to dispatches/pending/                              │
└────────────────────┬───────────────────────────────────────────┘
                     │
                     ▼ Human promotes
              [Operator reviews & approves]
                     ...
                     ▼
        ┌────────────────────────┐
        │  dispatches/active/    │◄── Promoted dispatches
        │  {timestamp}-{track}.md│
        └────────────┬───────────┘
                     │
...
### 1. dispatcher_v8_minimal.sh (V8 Native Skills)
**Purpose**: Routes dispatches to the correct worker terminal  
**Trigger**: When a dispatch is promoted to active/  
**Delivery**: Via tmux send-keys or subprocess adapter (per-terminal config)  
**ACK**: ACK receipts generated by `heartbeat_ack_monitor.py`

### 2. receipt_processor_v4.sh (Primary receipt delivery)
**Purpose**: Converts new markdown reports into receipts and delivers them into T0  
**Trigger**: Detects new `.vnx-data/unified_reports/*.md`  
**Writes**: Appends to `.vnx-data/state/t0_receipts.ndjson`  
**Delivery**: tmux paste-buffer + double Enter for reliable submission (receipts to T0)

### 3. vnx_supervisor_simple.sh
**Purpose**: Bulletproof process management and health monitoring  
**Trigger**: Continuous monitoring every 10 seconds  
**Monitors**: dispatcher, receipt_processor, heartbeat_ack_monitor, dashboard  
**Features**: Singleton supervisor, critical process restart, PID management  
**Locking**: Atomic directory creation (macOS compatible)

### 4. generate_valid_dashboard.sh
