...
- Create `scripts/lib/effectiveness_probe.py` with:
  ...
  - `PROBE_TO_BEACON` mapping from the probe vocabulary to `HealthBeacon`'s classification: `ok→ok`, `degraded→stale`, `produces_crap→fail`, `unknown→` (no beacon / `stale` with signal `"no probe"`). **A tampered/broken hash-chain maps to `fail`, NOT `corrupt`** (kimi/deepseek finding): `health_beacon.py` derives `corrupt` only from unreadable JSON and `all_beacons()` has no `status=='corrupt'→health` branch, so routing a probe tamper-signal to `corrupt` would fall through to staleness logic and misclassify. The governance probe emits `fail` for a broken chain and records `"tamper"` in the beacon `detail`; `corrupt` stays owned by the beacon layer for unparseable files.
    **D3a correction (2026-08-30):** the bolded claim above is now stale. D3a's gap-1 fix (`all_beacons()` never upgrading a self-reported non-`ok` status to `ok`) added exactly the `status=='corrupt'→health='corrupt'` branch this paragraph said didn't exist — the failure mode it warns about ("falls through to staleness logic and misclassify[es]") no longer happens for ANY self-reported status, not just `corrupt`. This does not retroactively make the governance probe's `fail`-for-tamper choice wrong; changing what the probe emits is a separate, probe-behavior decision this dispatch did not make. It only means the reason given here for never doing so is gone — `all_beacons()` is generic health-vocabulary; new health values live in `scripts/lib/health_beacon.py`'s own module docstring, not copied here.
