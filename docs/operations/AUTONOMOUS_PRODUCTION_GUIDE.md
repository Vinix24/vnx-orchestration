...
**Version**: 2.0
**Date**: 2026-03-09
**Status**: Specificatie (implementatie = fase 2)
...
| **Alleen Sonnet + Opus** | T1/T2 = Sonnet (standaard), T3 = Opus (complex). Geen Codex/Gemini |
...
| Check | Command | Criterium |
|-------|---------|-----------|
| T1 worktree | `test -d ../SEOcrawler_v2-wt-T1` | Exists |
| T2 worktree | `test -d ../SEOcrawler_v2-wt-T2` | Exists |
| T3 worktree | `test -d ../SEOcrawler_v2-wt-T3` | Exists |
| Git clean | `git -C <worktree> status --porcelain` | Empty |
| Branch sync | `git -C <worktree> rev-parse HEAD` vs `origin/main` | Up-to-date |
