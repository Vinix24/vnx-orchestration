...
| `mcp_servers` | Per-role allowlist of named MCP servers | **Hard-enforced** via a scoped `--mcp-config` (`resolve_role_mcp_config()`) — takes effect in scoped mode (the default), and only when `requires_mcp=False`. An empty list (the default; no shipped role currently declares one) keeps the `{"mcpServers":{}}` posture. A named server not defined in the ambient global config (`~/.claude.json`, or `VNX_GLOBAL_MCP_CONFIG_PATH` override) is skipped and logged, never fabricated. The config travels as an inline `--mcp-config <json>` argv element, so a server keeps its `env` and `headers` **only as `${VAR}` references** (`Bearer ${VAR}` counts): a literal value is dropped and logged by server and key, never by value, and a same-named variable exported to the worker still reaches a stdio server through environment inheritance. `args` and `url` are copied as written, so put a credential in `env` as a reference. Measurements: `scripts/lib/mcp_server_config.py` module docstring |
