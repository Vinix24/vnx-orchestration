...
F43 ("auto-handover at 65% context pressure for headless workers") is a feature that detects when a headless `claude -p` conversation is approaching the model's context window limit and triggers a handover dispatch — the worker writes a context summary, the orchestrator reads it, and dispatches a fresh worker that picks up from the same task. Without this, headless workers die abruptly when context fills up, losing in-progress work.
