...
**Date:** 2026-07-15 (revised 2026-07-16, twice — see Revision notes below)
...
**Revision (2026-07-16, round 2 — codex regate):** A follow-up adversarial pass on the revision above (codex, continuing the C-numbering) found three further issues in the per-epoch anchor design itself — the architecture from round 1 is **not reopened**, these are targeted refinements on top of it: **C6** — a per-epoch anchor (R4) pins only that epoch's SEAL POINT, its start; the epoch's own tail, between its `chain_epoch_start` marker and the next epoch's, stays unbound, and the next marker's reset to `GENESIS` never binds the epoch it closes — a local attacker can rewrite content strictly inside a still-open epoch, preserve line counts, recompute `prev_hash` for the rewritten suffix, and leave both surrounding markers untouched, and `verify_chain` still reports healthy. Closed by anchoring an epoch-CLOSURE commitment — the closing epoch's tail hash + line count at the moment the next epoch opens (or an operator forces an early re-seal) — as a new, sibling git record; every CLOSED epoch's exact tail is now pinned, not only its start, while the currently-open (latest) epoch keeps an explicitly-named accepted-residual open tail until its own closing seal (§1, §2, §4, §7; new T17). **C7** — `read_git_anchor` requires a known epoch, but the reverse fail-closed rule (R1/C1) needs to ask "does any anchor exist for this identity, at any epoch" precisely when the ledger is missing/reset and offers no epoch to look up — a question a per-epoch lookup cannot answer by construction. Added a full-identity scan, `read_git_anchors_for_identity`, and wired the reverse rule through it (§2, §7; new T18). **C8** — the anchor-immutability CI check (§3) blocks modifying or deleting an existing key but not adding a new, forged one; this residual is now named explicitly as a trust assumption of the PR-merge boundary rather than left implicit, because CI structurally has no ledger content to check a new key's fields against (the Hard Constraint above keeps ledgers local) — machine-checkable seal-provenance was considered and set aside for the same reason (§5; new T19).
...
**A per-epoch anchor alone still leaves that epoch's own tail unbound — closed with an epoch-closure commitment (design-review round 2, C6).** The per-epoch fix above (R4) pins `origin_hash`/`origin_line_number`/`entries_before_origin` — the epoch's *opening* fingerprint, computed at its `chain_epoch_start` marker. Nothing about that fingerprint constrains what happens between that marker and the next one: `append_epoch_marker` (`ndjson_hash_chain.py`) always resets `prev_hash` to `GENESIS` for a new epoch regardless of what preceded it, and `verify_chain`'s per-epoch walk only requires internal consistency *within* the observed chain — it never checks whether that chain is the SAME content that was there when the epoch was git-anchored. A local actor with ledger write access can therefore leave the anchored `chain_epoch_start` marker for epoch N untouched, rewrite or truncate any entries between it and epoch N+1's marker, recompute `prev_hash` for the rewritten suffix so it stays internally consistent, and preserve line counts so `entries_before_origin` for epoch N+1 (if already anchored) still matches. `verify_chain` sees a structurally valid, git-anchored epoch N start and an unaffected epoch N+1 start, and reports healthy — the entire body of epoch N was rewritten in between.

Fix: anchor an epoch's CLOSE, not only its open. When epoch N+1 is sealed (or an operator forces an early re-seal to close epoch N — see §4's `force_new_epoch`), `seal_and_commit_origin` computes epoch N's **closure fingerprint** — the hash and absolute line number of epoch N's last entry (the line immediately before epoch N+1's marker) plus the entry count that fell within epoch N — and commits it as a **new, sibling record** at `closure_key(identity, N) = f"{anchor_key(identity, N)}:close"` (§7). This is a brand-new key distinct from `{identity}#{N}`, so it is covered by §3's existing "only new keys may be appended" CI rule with no change to that rule's mechanism — a closure record is never a modification of the opening record, it is an additional, independently-immutable line. Every record, open or closure, carries an explicit `record_type: "open" | "close"` field so a reader need not parse the key string to tell them apart, though the key format alone is already sufficient for git's own diff/lookup purposes.

`verify_chain` then requires (§2): every epoch that has since CLOSED (a later epoch's opening anchor exists) must have a matching closure record whose `closure_hash`/`closure_line_number` reproduce exactly what the ledger shows at that line — any mismatch, including a rewritten-but-line-count-preserving suffix, is `"broken"`. Only the LATEST epoch — the one with no later epoch anchored yet — has no closure record, and its tail beyond the opening anchor is an **explicitly accepted residual, open until that epoch's own closing seal**, no wider than the window between two seals.

Because ADR-029 epochs otherwise only rotate when chaining lapses (`chain_epoch_seal.py::seal_ledger`'s existing idempotent check) — an event that may be rare, or may never recur once chaining is stable — `seal_and_commit_origin` gains an explicit `force_new_epoch` lever (§4, §7) so an operator can periodically close and reopen an epoch purely to shrink this residual window, independent of whether chaining has actually lapsed. This is the "periodic re-seal" alternative trigger considered alongside natural epoch rotation; both drive the same closure-commit mechanism, not two different schemas — a separate mid-epoch checkpoint record type (sequenced snapshots of a still-open epoch's tail) was considered and rejected as unneeded complexity beyond what closes this hole (see Consequences, Rejected).
...
- **A CLOSED epoch — one with a later epoch's opening anchor already resolved — has no matching closure record, or its closure record's `closure_hash`/`closure_line_number` doesn't reproduce what the ledger walk shows at that line** → `"broken"` (design-review round 2, C6 — see §1's epoch-closure note). This closes the gap R4 left open: R4 pinned every epoch's *start*; this pins every CLOSED epoch's *end* too, so content rewritten strictly between two anchored markers no longer verifies merely because both markers themselves are untouched. **The currently-open (latest) epoch is exempt** — it has no later epoch anchored yet, so it has no closure record to match against; its tail beyond the opening anchor is the accepted residual §1 names explicitly.
- **Anchor resolves and matches, for every OPEN epoch observed in the walk, and every CLOSED epoch's closure record matches too (C6)** → `"verified"` / `"verified-segmented"`, unchanged from ADR-029 otherwise.
...
**Closure records need no change to this rule (design-review round 2, C6).** `{ledger_identity}#{epoch}:close` (§1, §7 `closure_key`) is a distinct key per line from `{ledger_identity}#{epoch}`, so "first record per key wins, only brand-new keys may be appended" already covers it unchanged: a closure record is new the first (and only) time an epoch closes, exactly like an opening record is new the first time an epoch opens. **What this check does not, and structurally cannot, prove: that a brand-new key's content is genuine** — it only proves an *existing* key is unmodified. A hand-authored, well-formed new key merged through an ordinary PR passes this check by novelty alone; that residual is named explicitly in §5 (C8), not closed here.
...
- `seal_and_commit_origin` — takes the lock via `ledger_lock_path(ledger_path)` (**not** a bespoke `fcntl.flock(ledger_path, ...)`) across the entire read-origin → compute → anchor sequence:
  1. Inspect `epoch_state(ledger_path)`. Append a `chain_epoch_start` marker opening a new epoch when either **(a)** chaining has lapsed (existing ADR-029 idempotent trigger, unchanged), or **(b)** the caller passed `force_new_epoch=True` (design-review round 2, C6 — the periodic-re-seal lever: an operator can close and reopen an epoch purely to shrink the accepted-residual window of §1/§2, independent of whether chaining lapsed). Marker append is the existing ADR-029 `append_epoch_marker`, now itself lock-holding per above — `seal_and_commit_origin` must use a re-entrant/already-held acquisition here, not a second blocking `flock` call on the same fd from the same process.
  2. Compute the NEW epoch's opening fingerprint (§1) from the now-stable ledger tail. **If a PRIOR epoch was open** (this is not the ledger's first-ever seal), **also compute that prior epoch's closure fingerprint** (§1, §7 `compute_epoch_closure`) — the hash and absolute line number of its last entry (the line immediately before the new marker) plus its entry count. Both computations read the same lock-held, now-stable snapshot; there is no window between them for a writer to interleave.
  3. Commit + push **both** new records in the same commit/PR when step 2 produced two: the opening anchor for the new epoch at `anchor_key(identity, epoch)`, and the prior epoch's closure record at `closure_key(identity, prior_epoch)`. A first-ever seal has no prior epoch to close, so it commits only the opening record, as before.
...
- **A new, well-formed `{identity}#{epoch}` (or `:close`) key landed through a NORMAL, non-bypassing PR merge is a further residual, named explicitly here rather than left implicit (design-review round 2, C8).** §3's CI check enforces that an *existing* key cannot be modified or removed; it does not, and structurally cannot, attest that a *new* key's fields were actually produced by a real `seal_and_commit_origin` run, because this ADR's Hard Constraint (above) keeps the ledger local and never ships it to the repo — CI has no ledger content on hand to check a candidate `origin_hash`/`closure_hash` against. Anyone with ordinary merge rights on the project's repo — a reviewer who approves a seal-looking PR without independently re-deriving the fingerprint, or a legitimately-authenticated contributor account — could hand-author a plausible new epoch's anchor (or closure) record and land it exactly like a real seal, without bypassing branch protection at all. This is a genuine, accepted trust assumption of the design: **`seal_and_commit_origin` is the only sanctioned producer of anchor records, and this ADR relies on the same PR-review trust boundary ADR-027 already accepts for every other governed change** (a reviewer is trusted to confirm a seal PR actually came from the automated seal pipeline — e.g. by convention of title/body/branch-name — not to re-verify cryptographic content by hand). Machine-checkable seal-provenance (CI independently recomputing the fingerprint from ledger content) is deliberately not specified here: it would require CI to read the local ledger, which the Hard Constraint forbids. One partial, already-built backstop softens this residual without closing it: a forged anchor for an epoch the real ledger never reaches still fails closed — loudly, as `"broken"`, not silently — via §2's reverse-direction rule (extended by C7); but because §3's immutability rule also forbids *removing* a merged key, recovering from a forged key that did land requires an explicit operator/ADR-level exception, not a mechanism this ADR builds.
...
1. Ship `compute_epoch_fingerprint`, `compute_epoch_closure`, `seal_and_commit_origin`, `read_git_anchor`, `read_git_anchors_for_identity`, `ledger_lock_path`, `closure_key` — additive, `verify_chain`'s read path unchanged, `fabric-audit` check C unaffected.
...
def closure_key(identity: str, epoch: int) -> str:
    """'{anchor_key(identity, epoch)}:close' — the sibling key for EPOCH's
    CLOSURE commitment (design-review round 2, C6). A brand-new key distinct
    from anchor_key(identity, epoch), so it is covered by §3's existing
    "only new keys may be appended" CI rule with no change to that rule's
    mechanism. Written once, when the NEXT epoch opens (or a forced re-seal
    closes this one early — see force_new_epoch on seal_and_commit_origin) —
    never for the currently-open (latest) epoch, which has no closure record
    by design (§1, §2)."""
...
def compute_epoch_closure(ledger_path: Path, epoch: int, *, next_epoch_marker_line: int) -> ClosureFingerprint:
    """Pure computation of EPOCH's CLOSURE commitment (design-review round 2,
    C6): the hash and absolute line number of epoch's last entry — the line
    immediately before next_epoch_marker_line — plus the entry count that fell
    within epoch. Caller must hold ledger_lock_path(ledger_path)'s flock, same
    as compute_epoch_fingerprint, and calls this in the SAME critical section
    that opens the next epoch (§4) so both fingerprints read one consistent
    snapshot. Returns {ledger_identity, epoch, closure_hash,
    closure_line_number, entries_in_epoch}. Never called for the currently-
    open (latest) epoch — only for an epoch that IS closing on this call, i.e.
    when seal_and_commit_origin is about to open epoch + 1."""
...
def seal_and_commit_origin(
    ...
    *,
    ...
    closes the prior one, §4 step 1(b)) even when chaining hasn't lapsed —
    design-review round 2, C6's periodic-re-seal lever, letting an operator
    bound the open-epoch residual window on a cadence rather than waiting for
    an unpredictable lapse. When a prior epoch closes (lapse-triggered or
    forced), this call ALSO computes and commits that epoch's closure record
    (compute_epoch_closure, closure_key) in the SAME commit as the new epoch's
    opening record — never a separate seal. Returns SealResult{action:
    "sealed"|"noop", epoch, origin, branch_name, pr_url|None, closed_epoch:
    int|None} — closed_epoch names which epoch's closure record, if any, this
    call also committed. Raises on git/push failure rather than swallowing it
    — see §4 for why a failed push still leaves verify_chain correctly
    fail-closed."""
...
def read_git_anchor(
    ...
    *,
    ...
    """Resolves anchor_key(identity, epoch)'s record (kind="open", the default)
    or closure_key(identity, epoch)'s record (kind="close" — design-review
    round 2, C6) from GIT-COMMITTED content only (§2): live fetch + `git show
    <ref>:...`, falling back to the local remote-tracking ref on network
    failure. NEVER reads the working-tree file. Returns the FIRST matching
    record in file order; "corrupt" if more than one record matches the same
    key (dupes=broken, §3 — the rule is identical for open and close keys).
    None only when ref resolves cleanly with genuinely no record for this key
    — for kind="close" this is the expected, accepted-residual case for the
    currently-open (latest) epoch (§1, §2), not itself a fault. ALSO returns
...
def read_git_anchors_for_identity(
    ...
    *,
    ...
    """Full-identity scan (design-review round 2, C7): every record in the
    git-committed anchor file whose key matches '{identity}#*' (open) or
    '{identity}#*:close' (closure), across ALL epochs — regardless of which
    epoch, if any, the LEDGER currently claims. Exists because read_git_anchor
    requires a known epoch as input, and the reverse fail-closed rule (§2
    R1/C1) needs to ask "does ANY anchor exist for this identity" precisely
    when the ledger is missing/empty/reset and supplies no epoch to look up —
    a question read_git_anchor cannot answer by construction. Returns every
    matching record found (possibly empty) plus a single AnchorProvenance for
    the scan as a whole. An empty list from a cleanly-resolved ref means
    genuinely no anchor exists anywhere for this identity — distinct from a
    resolution failure, which still routes to "broken" per §2 step 3."""
...
def verify_chain(
    ...
    *,
    ...
    - Forward (open): whenever the walk observes a chain_epoch_start marker
      for epoch N, a matching git anchor for anchor_key(identity, N) is now
      REQUIRED — its absence, a resolution failure, or a "corrupt" (duplicate)
      anchor all report "broken". Checked for EVERY epoch observed, not only
      the first (§1/§2 R4 fix).
    - Forward (closed): whenever the walk observes that epoch N has CLOSED (a
      later epoch N+1's opening marker/anchor is present), a matching closure
      record at closure_key(identity, N) is now REQUIRED, and its
      closure_hash/closure_line_number must reproduce exactly what the walk
      observes at that line — mismatch or absence is "broken" (design-review
      round 2, C6). The currently-open (latest) epoch is exempt — it has no
      later epoch yet, so no closure record is expected; its tail beyond the
      opening anchor is the accepted residual §1 names explicitly.
    - Reverse: resolved BEFORE the empty/missing-ledger short-circuit, via
      read_git_anchors_for_identity (§7 C7) rather than a per-epoch lookup —
      the missing/reset case this guards against supplies no epoch number for
      read_git_anchor to target. If that scan finds any anchor (open or
      closure) for this ledger_identity at any epoch and the ledger walk does
      not independently confirm the corresponding epoch is chained, that is
      "broken" too (§2 R1/C1 fix, extended by C7). A missing/empty ledger is
      only "unchained" once this scan confirms no anchor exists for it
      anywhere.
...
#
# Closure records ('{ledger_identity}#{epoch}:close', §1 C6) are a distinct key
# per line and need no change to this rule: they are new keys the first time
# an epoch closes, exactly like an opening record is new the first time an
# epoch opens (§7 closure_key). Note (§5 C8): this check proves an existing
# key is unmodified — it cannot prove a brand-new key's content is genuine,
# since CI never has ledger content to check it against; that gap is an
# accepted trust assumption, not something this check closes.
```
...
| T17 | Seal epoch N and anchor it; seal epoch N+1 (committing epoch N's closure record); then rewrite/truncate ledger content strictly BETWEEN epoch N's `chain_epoch_start` marker and epoch N+1's marker (preserving line count), recompute `prev_hash` for the rewritten suffix, leaving both markers themselves untouched → `verify_chain` returns `"broken"` because the rewritten tail no longer reproduces epoch N's committed `closure_hash`/`closure_line_number` | C6 epoch-tail-binding (intra-epoch rewrite between two intact markers) |
| T18 | Ledger file entirely missing/reset while only an `{identity}#2` anchor exists in git (no epoch-1 anchor committed) → `read_git_anchors_for_identity` finds the epoch-2 record via a full-identity scan (no per-epoch lookup is possible since the empty ledger supplies no epoch) → `verify_chain` returns `"broken"`, not `"unchained"` | C7 reverse-check needs a scan API |
| T19 | A brand-new, well-formed `{identity}#{epoch}` key is hand-authored (not produced by `seal_and_commit_origin`) and merged via a normal, non-bypassing PR — the anchor-immutability CI check accepts it (it is a novel key, not a modification) — but `verify_chain` against the REAL ledger, which never reaches that epoch, still reports `"broken"` via the reverse-direction rule (T13/C7), not silently healthy | C8 trust-assumption — demonstrates the partial backstop, not a fix |
...
- Every CLOSED epoch now needs a second committed record — its closure (§1, §2 C6) — in addition to its opening anchor; the currently-open (latest) epoch is the sole exception, carrying an explicitly-accepted open tail until its own closing seal.
- `seal_and_commit_origin` gains a `force_new_epoch` parameter (§4, §7 C6) so an operator can bound that open-tail residual on a cadence, independent of whether chaining has actually lapsed — a new operator-facing lever, not only an internal implementation detail.
- A forged-but-novel anchor key merged through an ordinary (non-bypassing) PR is an accepted, explicitly-named trust assumption on the PR-review boundary (§5 C8), not a gap this design's CI mechanism closes — reusing the same reviewer-trust bar ADR-027 already accepts elsewhere.
...
- A periodic-checkpoint/sequence-numbered closure scheme for the still-open epoch (§1 C6) — considered and set aside as unneeded complexity beyond what the finding requires: the open epoch's tail is an explicitly accepted residual bounded by its own next seal, with `force_new_epoch` as the lever to shrink that window on demand, rather than a separate mid-epoch checkpoint record type.
- Machine-checkable seal-provenance for new anchor keys (§5 C8) — rejected for this phase: verifying a new key's fields against real content would require CI to read the local ledger, which the Hard Constraint (receipts stay local) forbids; accepted as a named trust assumption on the PR-review boundary instead.
