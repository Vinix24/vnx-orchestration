...
3. **Approve starting Phase 0 NOW?**
   **Resolved: YES — approved and largely landed.** The overnight mandate is the approval. Phase-0 progress:
   ...
   - **Hash-chain completion — DONE (PR #1090, ADR-029).** The check-C blast-radius blocker (epoch rotation) was the ratified fix: epoch-aware `verify_chain` (`verified-segmented` status), `chain_epoch_seal.py`, fabric-audit check C accepts segmented chains. 31 tests, codex-PASS.
