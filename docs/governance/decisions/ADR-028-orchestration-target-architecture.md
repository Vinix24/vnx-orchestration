...
3. **Approve starting Phase 0 NOW?**
   **Resolved: YES — approved and largely landed.** The overnight mandate is the approval. Phase-0 progress:
   ...
   - **Hash-chain — MECHANISM landed (PR #1090, ADR-029); default-on activation still pending.** The check-C blast-radius blocker (epoch rotation) was the ratified fix: epoch-aware `verify_chain` (`verified-segmented` status), `chain_epoch_seal.py`, fabric-audit check C accepts segmented chains. 31 tests, codex-PASS. NOT yet the full Phase-0 item: judge decisions do not yet chain exclusively via `append_chained_entry` and `VNX_CHAIN_RECEIPTS` stays default-OFF pending burn-in + running the seal — those are the remaining activation steps.
   - **Dual-write P5 cutover — VERIFIED COMPLETE (2026-07-10).** "Remove dual-write paths" here means the runtime cutover, not the `dual_writer.py` module deletion (that is Phase 6 P6 per its own docstring, and is the deliberate `VNX_DISPATCH_LEGACY=1` rollback net for the rest of Phase 0). Verified: `resolve_state_dir()` → central for every consumer; all live receipt ledgers write central (mission-control 1418, sales-copilot 447, vnx-dev 15.5k, seocrawler clean); the `dual_writer` mirror is a proven no-op everywhere (`primary == central` → cutover-skip). The last stale repo-local `.vnx-data/state/` (vnx-dev, frozen 2026-07-05) was salvaged (1 orphan receipt appended to central) and retired to `.pre-retirement-<ts>`. `vnx fabric-audit` GREEN before and after. The scaffolding removal (P6) waits until Phase-0 has burned in, because it is the rollback lever.
   - **Remaining Phase-0 item:** unknown-identity invalidation. Phase 0 is a correctness fix independent of the future architecture.
