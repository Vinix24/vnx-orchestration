...
cmd_remove() {
    ...
    # Check for unmerged .vnx-data (isolated model)
    ...
    if [ -d "$wt_data" ] && [ ! -L "$wt_data" ] && [ -f "$wt_data/.snapshot_meta" ]; then
        ...
        if [ "$report_count" -gt 0 ]; then
            ...
            echo "or use: vnx_worktree_setup.sh remove $name --force"
            if [ "${2:-}" != "--force" ]; then
                return 1
            fi
            echo "Force removal requested — data will be lost."
        fi
    fi
...
cmd_init_terminals() {
    ...
    for t in T1 T2 T3; do
        ...
        if [ -d "$wt_dir" ]; then
            ...
            echo "source_dir=$main_data" >> "$wt_data/.snapshot_meta"
            # Write .env_override for VNX_DATA_DIR isolation (Option 3B)
            cat > "$wt_data/.env_override" <<OVERRIDE
# Auto-generated by vnx_worktree_setup.sh init-terminals ($(date -u +%Y-%m-%dT%H:%M:%SZ))
export VNX_DATA_DIR='$wt_data'
OVERRIDE
            echo "[$t] Created: $wt_dir (branch: $branch, isolated .vnx-data)"
        fi
    done
}
