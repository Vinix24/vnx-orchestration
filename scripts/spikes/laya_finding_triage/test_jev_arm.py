#!/usr/bin/env python3
"""Unit-test voor de JEV-arm (03c_jev_classify.py).

De HTTP-laag wordt vervangen door een vastgelegd antwoord in de vorm die de
dispatch citeert. De test toont:

  1. parse_response vertaalt een vastgelegd antwoord correct.
  2. classify_finding geeft exact de Laya-sleutels (LAYA_KEYS) terug, met de
     juiste choice/confidence, en boekt het defect-signaal apart.
  ...
  7. Zonder TYPESAFE_API_KEY stopt main() netjes met exit 2 en schrijft geen
...
# Vastgelegd antwoord in de vorm die de dispatch citeert: answers per
# vraag-sleutel, een choice-antwoord met choice/confidence/probabilities en
# een noul-antwoord met choice/confidence, plus usage.input_tokens.
RECORDED_BODY = {
    "answers": {
        "categorie": {
            "choice": "beveiliging",
        ...
        "defect": {
            "choice": "ja",
            "confidence": 0.91,
            "probabilities": {"ja": 0.91, "nee": 0.09},
        },
    },
    "usage": {"input_tokens": 410},
}


def fake_http_post_ok(url, headers, payload, timeout):
    """Geeft het vastgelegde antwoord, enlegt dat de payload fan-out heeft."""
...
def test_parse_response_happy():
    ...
    check("defect.choice == ja", parsed["defect"]["choice"] == "ja")
    check("defect.confidence == 0.91", parsed["defect"]["confidence"] == 0.91)
...
def test_build_request_payload_fanout():
    ...
    check("model == jev-latest", payload["model"] == "jev-latest")
    q = payload["questions"]
    ...
    # Eis 2: defect is noul met ja/nee.
    dcrit = q["defect"]["criteria"]
    check("defect criteria ja/nee", set(dcrit.keys()) == {"ja", "nee"})
...
def test_classify_finding_keys_match_laya():
    ...
    cat, defect, latency_ms, input_tokens, error = jev.classify_finding(
    ...
    check("defect.choice == ja", defect["choice"] == "ja")
    check("defect.confidence == 0.91", defect["confidence"] == 0.91)
...
def test_unknown_choice_flagged():
    ...
    body = {
        "answers": {
            ...
            "defect": {"choice": "ja", "confidence": 0.9, "probabilities": {}},
        },
        "usage": {"input_tokens": 10},
    }
    jev.parse_response(body)
    cat, defect, latency_ms, input_tokens, error = jev.classify_finding(
        "x", "fake-key",
        http_post=lambda u, h, p, t: body
    )
    # main() zet choice=None bij een onbekende keuze; hier testen we de grondstof.
...
def test_main_no_key_exits_clean():
    """Droge run: zonder TYPESAFE_API_KEY stopt main() met exit 2 en schrijft niets."""
...
def test_main_no_key_does_not_print_value():
    ...
    # vorm die de waarde afdrukt (key:-). We checken de bewoording, niet de
    # letterlijke shell-expressie, want die laatste zou net de waarde-printen
    # die we juist afwijzen.
