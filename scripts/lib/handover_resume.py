...
def validate_handover(payload: Dict[str, Any]) -> Result:
    ...
    # Identity validation
    if not DISPATCH_ID_PATTERN.match(payload.get("dispatch_id", "")):
        errors.append(f"Invalid dispatch_id: {payload.get('dispatch_id')}")
    if not PR_ID_PATTERN.match(payload.get("pr_id", "")):
    ...
    # HO-3: next_action must have concrete recommendation
    ...
    for fm in summary.get("files_modified", []):
    ...
    # HO-4: residual_state must be present
    if "residual_state" not in payload:
        errors.append("residual_state section is required (HO-4)")
    else:
        rs = payload["residual_state"]
        for oi in rs.get("open_items_created", []):
            err = _validate_open_item(oi)
            if err:
                errors.append(err)

    # HO-5: context_for_next must include critical_context
    ctx = payload.get("context_for_next", {})
...
def validate_resume(payload: Dict[str, Any]) -> Result:
