#!/usr/bin/env python3
"""
VNX FP-C Certification Checks — Individual section verifiers.

Extracted from fpc_certification.py to keep module size manageable.
Provides FPCCertificationChecksMixin with all _certify_* methods
consumed by FPCCertificationRunner via inheritance.
"""
...
class FPCCertificationChecksMixin:
    """Mixin providing all _certify_* section methods for FPCCertificationRunner.

    Relies on self._state_dir, self._dispatch_dir, self._output_dir,
    self._quality_db_path, self._register_test_dispatch, self._pass,
    self._fail, self._skip being defined on the host class.
    """
