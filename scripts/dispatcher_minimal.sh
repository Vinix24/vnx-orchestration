...
if [ "${VNX_RESUME_IN_PROGRESS:-0}" != "1" ] && [ -f "${VNX_STATE_DIR}/PAUSED" ]; then
  echo "[dispatcher_minimal] PAUSED marker present at ${VNX_STATE_DIR}/PAUSED — refusing to start. Run 'vnx resume' to clear." >&2
  exit 0
fi
...
enforce_singleton "dispatcher_minimal"
...
source "$VNX_DIR/scripts/pane_manager.sh"
