#!/bin/bash
# receipt_processor.sh - Time-aware receipt processing with flood protection
...
if [ "${_RP_LIB_MODE:-0}" != "1" ] && [ "${VNX_RESUME_IN_PROGRESS:-0}" != "1" ] && [ -f "${VNX_STATE_DIR}/PAUSED" ]; then
  echo "[receipt_processor] PAUSED marker present at ${VNX_STATE_DIR}/PAUSED — refusing to start. Run 'vnx resume' to clear." >&2
  exit 0
fi
...
cleanup() {
    ...
    rm -f "$VNX_PIDS_DIR/receipt_processor.sh.pid" "$VNX_PIDS_DIR/receipt_processor.sh.pid.fingerprint"
}
