...
- **An empty provider completion is usually a TRANSIENT lane issue, not a structural door bug:** if `bin/vnx dispatch <id>` for a provider lane (kimi/glm/deepseek) lands an empty completion, check the lane's health FIRST — kimi OAuth/quota, glm proxy on :4141 — not the envelope. The door's provider path works (verified end-to-end; a `~1s` empty result on 2026-07-10 ~20:00 was a transient kimi auth/quota dip that recovered, NOT the envelope). Historically this looked structural because the fail-loud guard was missing: as of #1107 an rc=0 with a blank `completion_text` is downgraded to a loud `failure` (embedding the raw spawn result) for ALL provider spawns, and the door surfaces `EnvelopeResult.error` instead of a bare exit code — so the real cause is now visible.
