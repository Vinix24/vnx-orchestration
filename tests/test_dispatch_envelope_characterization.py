...
from dispatch_envelope_census_scanner import (
    ...
    Coupling,
    discover_family,
    discover_family_functions_and_classes,
...
_ALL_FUNCS, _ALL_CLASSES = discover_family_functions_and_classes()
...
class TestLayer4AnnotationResolution:
    ...
    Scoped to the whole envelope_* family via
    discover_family_functions_and_classes() — the SAME discover_family()
    input Layer 2's census scan and Layer 6's bind-form check use — instead
    of a fixed `__module__ == "dispatch_envelope"` filter. That fixed filter
    was the OI-1000 bug: the moment a symbol relocates to an envelope_*
    sibling, its __module__ reads the sibling's name, the filter drops it
    silently, and the parametrization loses a case with the suite staying
    green throughout. Family-scoping keeps the symbol covered under its
    real owning module (visible in the test id, e.g.
    envelope_types.EnvelopeSpec instead of dispatch_envelope.EnvelopeSpec)
    for every step of the PR-1..PR-6 split. See TestLayer4CaseCountFloor
    below for the belt-and-braces guard against any OTHER way this count
    could shrink silently (rename, scanner bug, ...).
    """
...
class TestLayer4CaseCountFloor:
    """A recorded floor on the raw Layer 4 symbol set, independent of the
    family-scoping fix above. Family-scoping alone does not protect against
    a symbol vanishing from the scan for an UNRELATED reason (renamed,
    module renamed out of the envelope_* pattern, a scanner bug) — that
    would shrink TestLayer4AnnotationResolution's case count just as
    silently as the original bug did, since a dropped parametrize case is
    not a failure, it is an absence.

    tests/data/dispatch_envelope_census.json's layer4_functions /
    layer4_classes keys are the recorded baseline — the SAME fixture file
    Layer 2's census uses, regenerated by the same
    `python3 tests/dispatch_envelope_census_scanner.py` command, so there is
    one expectation-file mechanism for this suite, not two. Unlike Layer 2's
    exact-match comparison, this check is asymmetric on purpose: growing the
    live set is free (a newly-added symbol needs no fixture update to stay
    green), but anything the fixture recorded that the live scan no longer
    finds fails loudly and is named in the assertion message.
    """

    def test_no_symbols_vanished_since_baseline(self):
        fixture = json.loads(CENSUS_FIXTURE.read_text(encoding="utf-8"))
        baseline_funcs = set(fixture.get("layer4_functions", []))
        baseline_classes = set(fixture.get("layer4_classes", []))
        live_funcs = {f"{owner}::{attr}" for (owner, attr) in _ALL_FUNCS}
        live_classes = {f"{owner}::{attr}" for (owner, attr) in _ALL_CLASSES}
        missing_funcs = sorted(baseline_funcs - live_funcs)
        missing_classes = sorted(baseline_classes - live_classes)
        if missing_funcs or missing_classes:
            lines = [
                "Layer 4 case count shrank vs tests/data/dispatch_envelope_census.json "
                "(layer4_functions/layer4_classes) — a symbol that used to resolve is "
                "gone from the scan:"
            ]
            if missing_classes:
                lines.append(f"  - {len(missing_classes)} class(es) no longer found:")
                lines.extend(f"      {c}" for c in missing_classes)
            if missing_funcs:
                lines.append(f"  - {len(missing_funcs)} function(s) no longer found:")
                lines.extend(f"      {c}" for c in missing_funcs)
            lines.append("  regenerate with: python3 tests/dispatch_envelope_census_scanner.py")
            pytest.fail("\n".join(lines))
