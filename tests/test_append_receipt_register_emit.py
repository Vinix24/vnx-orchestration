...
def test_enrich_sets_open_items_created_internally(ar):
    ...
    when a receipt already carries quality_advisory{} (pre-cutover/replay input).
    ...
    ADR-035 §9 PR-5: quality_advisory{} is no longer GENERATED by
    _enrich_completion_receipt (generate_quality_advisory is no longer
    called there at all -- superseded by verdict{}/warnings[], §6). This
    test now supplies quality_advisory{} directly on the input receipt to
    confirm _count_quality_violations_against_store's dry-run dedup logic
    itself is unchanged for a receipt that still carries the legacy shape.
    """
    receipt = {
        "timestamp": "2026-04-28T12:00:00Z",
        "event_type": "task_complete",
