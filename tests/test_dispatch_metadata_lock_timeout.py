...
Covers:
- ``upsert_dispatch_provider_row`` given a short explicit ``timeout=`` gives up
  well before the driver's 5s default when the DB is genuinely locked, measured
  by wall-clock elapsed time (not just "no exception escaped" — that was already
  true and is exactly what the finding says missed the actual point).
...
def test_short_timeout_gives_up_fast_under_lock(tmp_path):
    """A short explicit timeout must bound the wait well under both the
