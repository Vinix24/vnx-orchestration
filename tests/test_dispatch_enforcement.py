...
class TestD12ViaAndConstraint:
    ...
    def test_build_snapshot_via_headless_when_no_explicit_choice(self, tmp_path: Path) -> None:
        """allow_headless=False is not "the normal case" — claude_headless is the
        only claude lane, so build_runtime_snapshot's via must be 'headless' here
        too (not just for an explicit allow_headless=True).
        ...
        assert captured.get("via") == "headless", (
            f"expected via='headless' (the only claude lane), got {captured.get('via')!r}"
        )
