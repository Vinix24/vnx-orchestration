...
def vnx_init(args) -> int:
    ...
    _config_existed = config_path.exists()
    if not _config_existed or force:
        _atomic_write(
            config_path,
            ...
            "  # cost-optimized routing; set false or VNX_ROUTING_POLICY_ENABLED=0 to disable\n",
        )
        print(f"  {'updated' if _config_existed else 'created'} {config_path.relative_to(project_dir)}")
    else:
